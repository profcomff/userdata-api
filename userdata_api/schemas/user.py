...
class UserInterface:
    ...
    @staticmethod
    def __refresh_model(*, origin_model: type[Base], updated_model: type[Base]):
    ...
    async def _refresh_get_model(self, session: Session):
        """
    ...
    async def _refresh_update_model(self, session: Session):
    ...
    async def refresh(self, session: Session):
        await self._refresh_update_model(session)
        await self._refresh_get_model(session)
