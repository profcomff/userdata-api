...
Дернуть ручку `GET /user/{user_id}`. Информация возвращается в соответствии с переданными scopes.
...
Дернуть ручку `GET /user?users=...?categories=...`. Ручка закрыта за скоупом `userdata.info.admin`. Информация возвращается в соответствии с переданными scopes.

- Ручка доступна только админам. Возвращает информацию о указанных юзерах и категориях(указываются id категорий и id юзеров)
